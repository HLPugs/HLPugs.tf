 ...
 * @property {Date} timestamp - The timestamp where the pug was started.
 * @property {number|null} logsID - The logs.tf unique log id, if any.
 * @property {Team[]} teams - An array of teams that played in the pug.
 */
export class Pug {
    id: number;
    map: string;
    ...
    constructor(id: number, map: string, teams: Team[]) {
        if(Team.length != 2) throw new Error("Teams must only contain 2 Team objects.");
        else if(teams[0].color === teams[1].color) throw new Error("Teams must be opposing colors.");
        else {
            this.id = id;
            this.teams = teams;
