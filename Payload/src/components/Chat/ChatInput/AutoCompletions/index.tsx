...
class AutoCompletion extends React.Component<AutoCompletionsProps, {}> {
	completionsCollection = () => {
		if (this.props.emojiCompletions.length) {
			return (
				<>
					{this.props.emojiCompletions.map(
			...
			return (
				<>
					{this.props.mentionCompletions.map(
						(completion: string, index: number) =>
							this.createCompletion(index, completion)
					)}
				</>
			);
	...
	createCompletion = (index: number, completion: string | CompletionItem) => {
		return (
			<div
				className={
					this.props.autoCompleteIndex === index
						? 'autoCompletionItemHighlight'
						: ''
				}
				key={index}
	...
	createCompletionContent = (completion: string | CompletionItem) => {
		if (typeof completion === 'string') {
			...
			return (
				<>
					<span title={completion.customName} className="emoji-mart-emoji">
						<span
	...
	render() {
		if (
			this.props.emojiCompletions.length ||
			this.props.mentionCompletions.length
		) {
			return (
				<div id="autoCompletionHolder">{this.completionsCollection()}</div>
			);
		}
		return <div />;
	}
