export default class HttpClient {
  ...
  private readonly apiPrefix: string = process.env.NODE_ENV === 'production' ? '/api' : 'http://localhost:3001/api'; // make this reflect current api version
  ...
  private async requestWithRetry(url: string, tryCount: number | undefined = this.defaultTryCount, options: RequestInit | undefined = undefined): Promise<any> {
    try {
