...
export const getPlayer = (steamid: string): Promise<Player> => {
	return new Promise(resolve => {
		if (!players.has(steamid)) {
			logger.warn(
				`Player (${steamid}) was requested from the player map but was not found`
			);
		}
 ...
 */
export const addPlayer = (sessionid: string, steamid: string) =>
	players.set(steamid, sessionid);
 ...
 */
export const addPlayerDraftTFClass = async (
	steamid: string,
 ...
 */
export const removePlayerDraftTFClass = async (
	steamid: string,
	draftTFClass: DraftTFClass
) => {
 ...
 */
export const getAllPlayersDraftTFClass = (
	draftTFClass: DraftTFClass
): string[] => {
